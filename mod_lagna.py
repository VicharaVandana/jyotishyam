...
def planetary_positions(jd, place):
  ...
  for planet in planet_list:
    ...
    constellation = 1 + int(nirayana_long / 30)
    coordinates = to_dms(nirayana_long % 30)
    positions.append([get_planet_name(planet), retro, constellation, coordinates, nakshatra_pada(nirayana_long)])
...
def ascendant(jd, place):
  ...
  # 12 zodiac signs span 360°, so each one takes 30°
  # 0 = Mesha, 1 = Vrishabha, ..., 11 = Meena
  constellation = 1 + int(nirayana_lagna / 30)
...
def base_tests():
  jd = swe.julday(1991, 10, 8, 14 + 47/60. + 13/3600.)  #yyyy,mm,dd,time_24hr_format(hh + mm/60 + ss/3600)
  print(ascendant(jd, honavar))
  print(planetary_positions(jd, honavar))
...
def compute_lagnaChart():
  ...
  for planetname in data.lagna_planets:
    ...
    planet["house-num"] = housediff(lagna, gen.signnum(planet["sign"]))
